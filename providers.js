...
function resolveSpotlightCountry(settings) {
    let mode = settings.get_string('spotlight-mode');
    if (mode === 'auto') {
...
function resolveSpotlightLocale(settings) {
    let mode = settings.get_string('spotlight-mode');
    if (mode === 'auto') {
        let langNames = GLib.get_language_names();
        let parts = langNames[0].split('.')[0].split('_');
...
const spotlightProvider = {
    ...
    async fetchMetadata(settings, httpSession) {
        let country = resolveSpotlightCountry(settings);
        let locale = resolveSpotlightLocale(settings);
